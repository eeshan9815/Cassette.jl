...
function overdub_recurse_pass!(reflection::Reflection,
    ...
    for i in 1:n_method_args
        ...
        push!(overdubbed_codelocs, overdubbed_codelocs[end])
    ...
    Meta.partially_inline!(code_info.code, Any[], method.sig, static_params,
                           overdub_slot_count, length(overdubbed_code), :propagate)
    ...
    if is_tagging_enabled(context_type)
